...
export default function VoiceAssistantModal({ isOpen, onClose, onOpenGame }) {
  ...
  const answer = useMemo(() => {
    ...
    if (hit) return `${hit.name} ${t('voiceConnectedAs')} ${hit.relation}. ${hit.description}`;
    if (lower.includes('game')) return t('voiceGameReady');
    return t('voiceFallbackAnswer');
  }, [prompt, t]);
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              <Magnetic strength={0.15} className="flex-1">
                <button type="button" onClick={handleSpeak} className="btn btn-on-light w-full"><Volume2 className="w-4.5 h-4.5" /> {t('speakAnswer')}</button>
              </Magnetic>
              <button type="button" onClick={onOpenGame} className="btn btn-line flex-1" style={{ borderColor: 'var(--paper-line)', color: 'var(--paper-ink)' }}>
                <PlayCircle className="w-4.5 h-4.5" /> {t('openGame')}
              </button>
            </div>
          </motion.div>
        </motion.div>
