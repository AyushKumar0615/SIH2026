  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="relative min-h-screen flex flex-col justify-between page !max-w-3xl">
        <button type="button" onClick={onBack} className="btn btn-quiet !px-0 self-start" style={{ color: 'var(--ink)' }}><ArrowLeft className="w-4 h-4" /> Back</button>

        <div>
          <span className="eyebrow">Story {currentIndex + 1} of {MOCK_FAMILY_MEMORIES.length} · {activeStory.category}</span>
          ...
          <div className="flex items-center gap-6 mt-10">
            <button type="button" onClick={handlePrev} className="trigger-btn" title="Previous"><SkipBack className="w-4.5 h-4.5" /></button>
            <Magnetic strength={0.25}>
              <button type="button" onClick={() => setIsPlaying(!isPlaying)} className="btn btn-ember !rounded-full !px-8">
                {isPlaying ? (<><Pause className="w-4.5 h-4.5" /> Pause</>) : (<><Play className="w-4.5 h-4.5" /> Resume</>)}
              </button>
            </Magnetic>
            <button type="button" onClick={handleNext} className="trigger-btn" title="Next"><SkipForward className="w-4.5 h-4.5" /></button>
          </div>
        </div>
      </div>
    </div>
