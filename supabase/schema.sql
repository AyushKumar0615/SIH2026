...
-- Two tables: elder_location_settings (one row per elder, tracking
-- opt-in + last known browser permission state so a later login can
-- auto-resume tracking instead of asking again) and elder_locations
-- (an append-only log of location samples — the caregiver/admin views
-- always read the single most recent row per elder). RLS mirrors the
-- existing caregiver_connections pattern exactly: an elder owns their
-- own rows, a caregiver may read only for elders they have an
...
-- Completed cognitive-game sessions for the caregiver Cognitive Analytics view.
create table if not exists public.game_sessions (
  ...
  analysis_status text not null default 'pending' check (analysis_status in ('pending', 'processing', 'ready', 'failed')),
  analysis jsonb,
  analysis_error text,
...
create policy "caregiver select connected elder game sessions" on public.game_sessions for select to authenticated using (exists (select 1 from public.caregiver_connections cc where cc.elder_id = game_sessions.user_id and cc.caregiver_id = (select auth.uid()) and cc.status = 'accepted'));
